...
短时互动课程建议优先用浏览器内置朗读。它可以朗读页面中新生成的句子，也不需要提前准备音频文件。

如果使用 edge-tts，应该先生成音频清单，覆盖单词、完整句子、题目、对话和示范回答。

### 真实反馈

Skill 应先判断输入是否有效，再给反馈。

如果用户只输入随机字母，Skill 应说明暂时无法评分，并要求用户提交真实答案。

对有效答案，反馈聚焦三点：

1. 最影响理解或得分的问题。
2. 重复出现的语法、词汇、连贯性或表达模式。
3. 一个可以立刻复练的表达升级。
...
如果 Agent 环境支持记忆系统，Skill 会询问是否保存这条学习记录。
...
edge-tts 适合固定音频包。动态课程通常更适合浏览器朗读。

### Azure AI Speech 和 Amazon Polly

只有在用户选择对应服务并提供凭证后再配置。

不要把凭证写入 HTML、README、日志、学习记录、记忆记录或提交内容。
